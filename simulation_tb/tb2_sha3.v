//======================================================================
// tb_sha3.v — SHA3 Testbench hoàn chỉnh (Groups 1-15)
//
// Group  1: Empty message
// Group  2: Single byte 0x00
// Group  3: Single byte 0xFF
// Group  4: Single byte 'a' (0x61)
// Group  5: "abc" (3 bytes)
// Group  6: a-z lowercase (26 bytes)
// Group  7: Exact block boundary (0xA3 x 136/72 bytes)
// Group  8: Long 200x 0xA3
// Group  9: Long 200x 0x00
// Group 10: "Hello, World!" — chữ hoa + thường + dấu câu
// Group 11: "SHA3-256"      — chữ hoa + hyphen + số
// Group 12: "Test@2024#!"   — mix + số + ký tự đặc biệt
// Group 13: A-Z uppercase (26 bytes)
// Group 14: "!@#$%^&*()"   — ký tự đặc biệt thuần
// Group 15: Pangram (43 bytes) — tất cả 26 chữ cái
//======================================================================
...
module tb2_sha3 ();
    ...
    // Hell=48656C6C  o,W=6F2C2057  orld=6F726C64  !06__=21060000
    ...
    // --- Group 11: "SHA3-256" (8 bytes, multiple of 4) ---
    // SHA3=53484133  -256=2D323536  0x06____=06000000
    ...
    // Test=54657374  @202=40323032  4#!06=34232106
    ...
    // --- Group 13: A-Z uppercase (26 bytes) ---
    // ABCD EFGH IJKL MNOP QRST UVWX YZ06__
    ...
    // --- Group 14: "!@#$%^&*()" (10 bytes) ---
    // !@#$=21402324  %^&*=255E262A  ()06_=28290600
    ...
    // The =54686520 quic=71756963 k br=6B206272 own =6F776E20
    // fox =666F7820 jump=6A756D70 s ov=73206F76 er t=65722074
    // he l=6865206C azy =617A7920 dog6=646F6706
    ...
    // addr mapping: tb_addr[6:0] -> RTL addr[8:2] LSB-aligned
    //   tb_addr[0] -> addr[2] (low/high half of 64-bit lane)
    //   tb_addr[5:1] -> addr[7:3] (lane index = word/2)
    ...
    task write_word;
        ...
        begin
            ...
            tb_wr_data=data; tb_we=1; #(CLK_PERIOD);
        end
    endtask
    ...
    // ------ test_empty ------
    task test_empty;
        input [511:0] ref; input integer bsz, osz;
        begin
            $display("*** Empty msg -- %0d-bit", osz);
            block_words=bsz>>5; output_words=osz>>5; #(4*CLK_PERIOD);
            for(i=0;i<50;i=i+1)
                if(i==0)            write_word(i,32'h06000000);
                else if(i==block_words-1) write_word(i,32'h00000080);
                else                write_word(i,32'h00000000);
            tb_we=0; tb_addr=0; pump_init(); check(ref);
        end
    endtask

    // ------ test_1byte ------
    task test_1byte;
        input [511:0] ref; input integer bsz, osz; input [7:0] b;
        begin
            $display("*** 1 byte 0x%02h -- %0d-bit", b, osz);
            block_words=bsz>>5; output_words=osz>>5; #(4*CLK_PERIOD);
            for(i=0;i<50;i=i+1)
                if(i==0)            write_word(i,{b,8'h06,16'h0000});
                else if(i==block_words-1) write_word(i,32'h00000080);
                else                write_word(i,32'h00000000);
            tb_we=0; tb_addr=0; pump_init(); check(ref);
        end
    ...
    task test_abc;
        input [511:0] ref; input integer bsz, osz;
        begin
            $display("*** abc -- %0d-bit", osz);
            block_words=bsz>>5; output_words=osz>>5; #(4*CLK_PERIOD);
            for(i=0;i<50;i=i+1)
                if(i==0)            write_word(i,32'h61626306);
                else if(i==block_words-1) write_word(i,32'h00000080);
                else                write_word(i,32'h00000000);
            tb_we=0; tb_addr=0; pump_init(); check(ref);
        end
    ...
    task test_az_lower;
        input [511:0] ref; input integer bsz, osz;
        begin
            $display("*** a-z lowercase (26B) -- %0d-bit", osz);
            block_words=bsz>>5; output_words=osz>>5; #(4*CLK_PERIOD);
            for(i=0;i<50;i=i+1)
                case(i)
                    0: write_word(i,32'h61626364);
                    1: write_word(i,32'h65666768);
                    2: write_word(i,32'h696a6b6c);
                    3: write_word(i,32'h6d6e6f70);
                    4: write_word(i,32'h71727374);
                    5: write_word(i,32'h75767778);
                    6: write_word(i,32'h797a0600);
                    default: if(i==block_words-1) write_word(i,32'h00000080);
                             else                 write_word(i,32'h00000000);
                endcase
            tb_we=0; tb_addr=0; pump_init(); check(ref);
        end
    ...
    // ------ test_exact_blk ------
    // message fills exactly 1 block → padding forced into block 2
    task test_exact_blk;
        input [511:0] ref; input integer bsz, osz;
        begin
            $display("*** Exact block (0xA3 x %0dB) -- %0d-bit", bsz>>3, osz);
            block_words=bsz>>5; output_words=osz>>5; #(4*CLK_PERIOD);
            for(i=0;i<50;i=i+1)
                if(i<block_words) write_word(i,32'hA3A3A3A3);
                else              write_word(i,32'h00000000);
            ...
            tb_we=0; tb_addr=0; pump_next(); check(ref);
        end
    endtask

    // ------ test_long_byte ------
    // 200 bytes of repeated byte value (multi-block)
    task test_long_byte;
        input [511:0] ref; input integer bsz, osz; input [7:0] val;
        reg [31:0] fill; reg done, padded;
        begin
            $display("*** Long 200x 0x%02h -- %0d-bit", val, osz);
            fill = {val,val,val,val};
            block_words=bsz>>5; output_words=osz>>5;
            #(4*CLK_PERIOD);
            total_bits=1600;
            for(i=0;i<50;i=i+1) begin
                if(i<block_words) begin write_word(i,fill); total_bits=total_bits-32; end
                else              write_word(i,32'h00000000);
            end
            tb_we=0; tb_addr=0; pump_init();
            done=0; padded=0;
            while(!done) begin
                for(i=0;i<block_words;i=i+1) begin
                    if(total_bits>0) begin
                        write_word(i,fill); total_bits=total_bits-32;
                    end else if(!padded) begin
                        if(i==block_words-1) begin write_word(i,32'h06000080); done=1; end
                        else write_word(i,32'h06000000);
                        padded=1;
                    end else if(i==block_words-1) begin
                        write_word(i,32'h00000080); done=1;
                    end else write_word(i,32'h00000000);
                end
                tb_we=0; tb_addr=0; pump_next();
            end
            check(ref);
        end
    endtask
    ...
    // ------ test_fixed_words ------
    // Test message đã encode sẵn vào mảng w[], n words
    // Gọi: set w[0..n-1] trước, rồi gọi task này
    reg [31:0] w[0:15];
    integer    wn;

    task test_fixed_words;
        input [511:0] ref; input integer bsz, osz;
        begin
            block_words=bsz>>5; output_words=osz>>5; #(4*CLK_PERIOD);
            for(i=0;i<50;i=i+1)
                if(i<wn)            write_word(i,w[i]);
                else if(i==block_words-1) write_word(i,32'h00000080);
                else                write_word(i,32'h00000000);
            tb_we=0; tb_addr=0; pump_init(); check(ref);
        end
    ...
    initial begin : sha3_test
        ...
        $display("==============================================");
        $display("  SHA3 Full Testbench (Groups 1-15)");
        $display("==============================================\n");
        num_err=0; init_sim(); reset_dut();

        // ---- G1: Empty ----
        $display("=== G1: Empty message ===");
        test_empty(H224_EMPTY,`SHA3_224_BLOCK_BITS,`SHA3_224_OUTPUT_BITS);
        test_empty(H256_EMPTY,`SHA3_256_BLOCK_BITS,`SHA3_256_OUTPUT_BITS);
        test_empty(H384_EMPTY,`SHA3_384_BLOCK_BITS,`SHA3_384_OUTPUT_BITS);
        test_empty(H512_EMPTY,`SHA3_512_BLOCK_BITS,`SHA3_512_OUTPUT_BITS);

        // ---- G2: 0x00 ----
        $display("=== G2: Single byte 0x00 ===");
        test_1byte(H224_B00,`SHA3_224_BLOCK_BITS,`SHA3_224_OUTPUT_BITS,8'h00);
        test_1byte(H256_B00,`SHA3_256_BLOCK_BITS,`SHA3_256_OUTPUT_BITS,8'h00);
        test_1byte(H384_B00,`SHA3_384_BLOCK_BITS,`SHA3_384_OUTPUT_BITS,8'h00);
        test_1byte(H512_B00,`SHA3_512_BLOCK_BITS,`SHA3_512_OUTPUT_BITS,8'h00);

        // ---- G3: 0xFF ----
        $display("=== G3: Single byte 0xFF ===");
        test_1byte(H224_BFF,`SHA3_224_BLOCK_BITS,`SHA3_224_OUTPUT_BITS,8'hFF);
        test_1byte(H256_BFF,`SHA3_256_BLOCK_BITS,`SHA3_256_OUTPUT_BITS,8'hFF);
        test_1byte(H384_BFF,`SHA3_384_BLOCK_BITS,`SHA3_384_OUTPUT_BITS,8'hFF);
        test_1byte(H512_BFF,`SHA3_512_BLOCK_BITS,`SHA3_512_OUTPUT_BITS,8'hFF);

        // ---- G4: 'a' ----
        $display("=== G4: Single byte 'a' ===");
        test_1byte(H224_BA,`SHA3_224_BLOCK_BITS,`SHA3_224_OUTPUT_BITS,8'h61);
        test_1byte(H256_BA,`SHA3_256_BLOCK_BITS,`SHA3_256_OUTPUT_BITS,8'h61);
        test_1byte(H384_BA,`SHA3_384_BLOCK_BITS,`SHA3_384_OUTPUT_BITS,8'h61);
        test_1byte(H512_BA,`SHA3_512_BLOCK_BITS,`SHA3_512_OUTPUT_BITS,8'h61);

        // ---- G5: "abc" ----
        $display("=== G5: abc ===");
        test_abc(H224_ABC,`SHA3_224_BLOCK_BITS,`SHA3_224_OUTPUT_BITS);
        test_abc(H256_ABC,`SHA3_256_BLOCK_BITS,`SHA3_256_OUTPUT_BITS);
        test_abc(H384_ABC,`SHA3_384_BLOCK_BITS,`SHA3_384_OUTPUT_BITS);
        test_abc(H512_ABC,`SHA3_512_BLOCK_BITS,`SHA3_512_OUTPUT_BITS);

        // ---- G6: a-z ----
        $display("=== G6: a-z lowercase ===");
        test_az_lower(H224_AZ,`SHA3_224_BLOCK_BITS,`SHA3_224_OUTPUT_BITS);
        test_az_lower(H256_AZ,`SHA3_256_BLOCK_BITS,`SHA3_256_OUTPUT_BITS);
        test_az_lower(H384_AZ,`SHA3_384_BLOCK_BITS,`SHA3_384_OUTPUT_BITS);
        test_az_lower(H512_AZ,`SHA3_512_BLOCK_BITS,`SHA3_512_OUTPUT_BITS);

        // ---- G7: Exact block boundary ----
        $display("=== G7: Exact block boundary ===");
        test_exact_blk(H256_EXB,`SHA3_256_BLOCK_BITS,`SHA3_256_OUTPUT_BITS);
        test_exact_blk(H512_EXB,`SHA3_512_BLOCK_BITS,`SHA3_512_OUTPUT_BITS);
        ...
        output_words=`SHA3_224_OUTPUT_BITS>>5; block_words=`SHA3_224_BLOCK_BITS>>5;
        #(4*CLK_PERIOD);
        for(i=0;i<50;i=i+1)
            if(i<wn) write_word(i,w[i]);
            else if(i==block_words-1) write_word(i,32'h00000080);
            else write_word(i,32'h00000000);
        tb_we=0; tb_addr=0; pump_init(); check(H224_HW);

        $display("*** Hello World -- 256-bit");
        output_words=`SHA3_256_OUTPUT_BITS>>5; block_words=`SHA3_256_BLOCK_BITS>>5;
        #(4*CLK_PERIOD);
        for(i=0;i<50;i=i+1)
            if(i<wn) write_word(i,w[i]);
            else if(i==block_words-1) write_word(i,32'h00000080);
            else write_word(i,32'h00000000);
        tb_we=0; tb_addr=0; pump_init(); check(H256_HW);

        $display("*** Hello World -- 384-bit");
        output_words=`SHA3_384_OUTPUT_BITS>>5; block_words=`SHA3_384_BLOCK_BITS>>5;
        #(4*CLK_PERIOD);
        for(i=0;i<50;i=i+1)
            if(i<wn) write_word(i,w[i]);
            else if(i==block_words-1) write_word(i,32'h00000080);
            else write_word(i,32'h00000000);
        tb_we=0; tb_addr=0; pump_init(); check(H384_HW);

        $display("*** Hello World -- 512-bit");
        output_words=`SHA3_512_OUTPUT_BITS>>5; block_words=`SHA3_512_BLOCK_BITS>>5;
        #(4*CLK_PERIOD);
        for(i=0;i<50;i=i+1)
            if(i<wn) write_word(i,w[i]);
            else if(i==block_words-1) write_word(i,32'h00000080);
            else write_word(i,32'h00000000);
        tb_we=0; tb_addr=0; pump_init(); check(H512_HW);

        // ---- G11: "SHA3-256" (8 bytes, len%4==0 → extra 0x06 word) ----
        // SHA3=53484133  -256=2D323536  0x06____=06000000
        $display("=== G11: SHA3-256 (uppercase+hyphen+digits) ===");
        ...
        tb_we=0; tb_addr=0; pump_init(); check(H256_S3);
        ...
        tb_we=0; tb_addr=0; pump_init(); check(H512_S3);

        // ---- G12: "Test@2024#!" (11 bytes) ----
        // Test=54657374  @202=40323032  4#!06=34232106
        $display("=== G12: Test@2024#! (mixed+numbers+special) ===");
        ...
        tb_we=0; tb_addr=0; pump_init(); check(H256_SP);
        ...
        tb_we=0; tb_addr=0; pump_init(); check(H512_SP);
        ...
        tb_we=0; tb_addr=0; pump_init(); check(H256_AZU);

        $display("*** A-Z uppercase -- 512-bit");
        output_words=`SHA3_512_OUTPUT_BITS>>5; block_words=`SHA3_512_BLOCK_BITS>>5;
        #(4*CLK_PERIOD);
        for(i=0;i<50;i=i+1)
            if(i<wn) write_word(i,w[i]);
        ...
        tb_we=0; tb_addr=0; pump_init(); check(H512_AZU);

        // ---- G14: "!@#$%^&*()" (10 bytes) ----
        // !@#$=21402324  %^&*=255E262A  ()06_=28290600
        $display("=== G14: !@#$%%^&*() pure special chars ===");
        ...
        tb_we=0; tb_addr=0; pump_init(); check(H256_PC);
        ...
        tb_we=0; tb_addr=0; pump_init(); check(H512_PC);

        // ---- G15: Pangram (43 bytes) ----
        // "The quick brown fox jumps over the lazy dog"
        $display("=== G15: Pangram (43 bytes, all 26 letters) ===");
        w[ 0]=32'h54686520; // The
        w[ 1]=32'h71756963; // quic
        w[ 2]=32'h6B206272; // k br
        w[ 3]=32'h6F776E20; // own
        w[ 4]=32'h666F7820; // fox
        w[ 5]=32'h6A756D70; // jump
        w[ 6]=32'h73206F76; // s ov
        w[ 7]=32'h65722074; // er t
        w[ 8]=32'h6865206C; // he l
        w[ 9]=32'h617A7920; // azy
        w[10]=32'h646F6706; // dog+0x06
        wn=11;
        ...
        tb_we=0; tb_addr=0; pump_init(); check(H256_PG);
        ...
        tb_we=0; tb_addr=0; pump_init(); check(H512_PG);
